 ...
 */
function calcEmLoc(targetElement) {
	...
	for(var j = 0; j < targetElement.childNodes.length; j++) {
		...
		if(targetElement.childNodes.item(j).nodeType == 1) {
			//	emLoc: [시작위치,끝위치]
			var emLoc = [];
			//	강조 시작위치
			emLoc.push(textPosition);
			//	강조 끝위치
			emLoc.push(textPosition + targetElement.childNodes.item(j).textContent.length - 1);

			//강조위치 배열에 추가
			emLocArray.push(emLoc);
		}
		//커서위치 
		textPosition += targetElement.childNodes.item(j).textContent.length;
	}
