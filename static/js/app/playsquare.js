...
function handleAnswerLike(obj, thisObj){
	$.ajax({
		...
		success: function(result){
			successHandleAnswerLike(result, thisObj);
		},
		error: function(e){
...
function followStack(obj, thisObj){
	$.ajax({
		...
		contentType: "application/json;charset=UTF-8",
		success: function(result){
			successFollowStack(result, thisObj);
		},
		error: function(e){
			alert("처리중 에러가 발생하였습니다.\n"+e.responseText);
			return;
 ...
 */
async function uploadAttachment(files, editor) {
	// 클라이언트에서 이미지 압축 실행

	var compressResult;
	var options = {
		targetSize: 1.5,
		maxWidth: Math.min(editor.width(),800)
	}
	if(typeof compressImageFile != 'function'){
		$.getScript('/js/public/compress.js', async function(){
			processAllFiles();
		});
	}else{
		processAllFiles();
	...
	// 주어진 파일들을 모두 압축 처리 및 업로드 시도
	function processAllFiles() {
		$(files).each(async function(i, file){
			compressResult = compressImageFileWithoutEvent([[file]],options);
			compressResult.then(function(result){
				uploadCompress(result);
			}).catch(function(msg){
				alert(msg + '<br>파일명:' + file.name);
			})
		})		
	}
	
	// 압축된 이미지를 서버 업로드 후 에디터 본문에 태그 삽입
	function uploadCompress(imgFile){
		...
		$.ajax({
	        ...
	        success: function (resourceUrl) {
	        	if(resourceUrl == null || resourceUrl.startsWith('NOT_ALLOWED')){
	        		...
	        		//-----------------------------------
	        		successUploadAttachment(resourceUrl, editor, imgFile.value.name);
	        		//-----------------------------------
	        	}
	        },
	        error: function (xhr) {
